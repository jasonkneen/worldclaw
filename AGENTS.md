...
- **AI features (chat, images, video, voice):** when `XAI_API_KEY` is in the
  env, the app has real xAI API access (server-only; latest model `grok-4.6`,
...
ai:        XAI_API_KEY in env → real xAI API: chat (grok-4.6) + Imagine image/video + voice TTS (docs.x.ai) → .grok/skills/xai-api/SKILL.md

...
- **Chat / LLM** — **latest model: `grok-4.6`**; default to it unless the
  user asks otherwise.
...
scene art, generated content). Distinct from your build-time `image_gen` /
`image_edit` tools (the `imagine` skill): use the **API** when the *running
app* generates media, the tools when *you* create static assets while building.
